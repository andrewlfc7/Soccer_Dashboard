...
def plot_goals_prob(ax, team_id, color,data):
...
def simulate_and_plot_match_result(ax,colors,shot_df, k=10000):
    ...
    ax.barh(labels, [result['home_prob'], result['draw_prob'], result['away_prob']], color=colors )
...
def plot_score_probability_matrix(ax, cmap, data):
    ...
    matrix = simulate_matchshots_with_matrix(shot_df=data, max_goals=4)

    # Create the plot
    sns.heatmap(matrix, cmap=cmap, ax=ax, square=True, annot=True, fmt='.1%', cbar=False)
